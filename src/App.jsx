...
function AppInner() {
  ...
  useEffect(() => {
    ...
    const handleUnload = () => setOnline(user.uid, false);
    window.addEventListener("beforeunload", handleUnload);
    return () => {
      setOnline(user.uid, false);
  ...
  return (
    ...
    >
      ...
      {/* ── Header ── */}
      {!selectedGoalId && !showCreateGoal && !selectedChannel && !showChannelCreate && (
      ...
      <main className="pb-20">
        ...
        {/* ── Chat ── */}
        {activeTab === 'chat' && !selectedChannel && !showChannelCreate && user && (
          <ChatList
            user={user}
            darkMode={darkMode}
            onSelectChannel={setSelectedChannel}
            onCreateChannel={() => setShowChannelCreate(true)}
          />
        )}

        {activeTab === 'chat' && showChannelCreate && user && (
          <ChannelCreate
            user={user}
            darkMode={darkMode}
            onBack={() => setShowChannelCreate(false)}
            onChannelCreated={(ch) => {
              setShowChannelCreate(false);
        ...
        {activeTab === 'chat' && selectedChannel && user && (
          <ChatView
      ...
      {/* ── Paywall Modal ── */}
      <PaywallModal open={showPaywall} onClose={() => setShowPaywall(false)} darkMode={darkMode} />
    </div>
  );
}
